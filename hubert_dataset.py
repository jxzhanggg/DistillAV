...
def parse_path(path):
    if path.endswith('.mp4'):
        path = path.replace('/video/', '/resized_video/')
        
    return path.replace('/yrfs4/', '/train8/').replace("jxzhang27", 'jxzhang46')

NUM=3000000000000000000000000000000
...
def load_audio_visual(manifest_path, max_keep, min_keep, frame_rate, label_paths, label_rates, tol=0.1):
    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                video_path = parse_path(items[1])
                audio_path = parse_path(items[2])
                audio_id = items[0]
    ...
    tot = ind + 1
    #print(f'nshort {n_short} nlong {n_long} n_unaligned {n_unaligned} label_rate {label_rate}')
    logger.info(
        (
...
def load_label_offset(label_path, inds, tot):
    with open(label_path) as f:
        ...
        code_lengths = [len(line.encode("utf-8")) for i, line in enumerate(f) if i <= NUM]
        assert (
...
def verify_label_lengths(
    ...
    with open(label_path) as f:
        lengths = [len(line.rstrip().split()) for i, line in enumerate(f) if i <= NUM]
...
class AVHubertDataset(FairseqDataset):
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            conformer_asr_fea = np.load(audio_fn.replace("/audio/", '/asr_model_fea/layer_15/').replace('.wav', '.npy'))

            # clean_fb80 = np.load(audio_fn.replace("/audio/", '/fb80/').replace('.wav', '.fb80.npy'))
            wavlm_fea_path = audio_fn.replace("/audio/", '/wavlm_model_fea/layer_15_full_avg8/').replace('.wav', '.npy')
            asr_fea = np.load(wavlm_fea_path) # t, d
        ...
        return video_feats, audio_feats, asr_fea, conformer_asr_fea
    ...
    def select_noise(self):
        ...
        for x in rand_indexes:
            ...
            noise_wav.append(wavfile.read(parse_path(self.noise_wav[x]))[1].astype(np.float32))
    ...
    def __getitem__(self, index):
        video_feats, audio_feats, asr_fea, conformer_asr_fea = self.load_feature(self.names[index])
        ...
        asr_fea = torch.from_numpy(asr_fea.astype(np.float32)) if asr_fea is not None else None
        conformer_asr_fea = torch.from_numpy(conformer_asr_fea.astype(np.float32)) if conformer_asr_fea is not None else None

        if video_feats is None and ("video" in self.modalities):
            fid = self.names[index][1].split(':')[1]
            logger.info(f'WARNING: index {index} video {fid} is None.')
            return {"id": None}
        ...
        fid = self.names[index][1].split(':')[1]
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, "asr_fea": asr_fea, "conformer_asr_fea":conformer_asr_fea}
    ...
    def collater(self, samples):
        ...
        # fea_source = [s['tar_fea'] for s in samples]
        # clean_fb80 = [s["clean_fb80"] for s in samples]
        asr_fea = [s["asr_fea"] for s in samples]
        conformer_asr_fea = [s["conformer_asr_fea"] for s in samples]
        ...
        assert(self.pad_audio==True)
        
        if self.pad_audio:
            audio_size = min(max(audio_sizes), self.max_sample_size)
            # if clean_fb80 is not None:
            #     fb80_size = max([len(c) for c in clean_fb80])
            #     if (fb80_size % 16) != 0:
            #         fb80_size = (fb80_size // 16 + 1) * 16  
        ...
        # [b, f, t]
        assert(np.all(np.array(audio_starts)==0)), f"audio_starts {audio_starts}"
        # collated_fea, _, __ = self.collater_audio(fea_source, audio_size, audio_starts)

        if asr_fea is not None:
            # collated_fb80, fb80_padding_mask, _ = self.collater_audio(
            #     clean_fb80,
            #     fb80_size,
            #     None)
            # fb80_padding_mask = (~fb80_padding_mask).float()
            collated_asr_fea, asr_fea_padding_mask, _ = self.collater_audio(
                asr_fea,
                audio_size * 2,
                None)
            collated_conformer_asr_fea, conformer_padding_mask, _ = self.collater_audio(
                conformer_asr_fea,
                audio_size,
                None
            )
            ...
            # collated_fb80, fb80_padding_mask = None,None
            collated_asr_fea = None
            collated_conformer_asr_fea = None
            asr_fea_padding_mask = None
            conformer_padding_mask = None
        ...
        source = {"audio": collated_audios, "video": collated_videos, 
                "asr_fea": collated_asr_fea,"asr_fea_padding_mask":asr_fea_padding_mask, 
                "conformer_asr_fea":collated_conformer_asr_fea, "conformer_padding_mask":conformer_padding_mask}
        ...
        # batch['tar_fea'] = collated_fea.transpose(1, 2) # [b, f, t] -> [b, t, f]
    ...
    def collater_frm_label(
        ...
        assert label_rate > 0
        # sample rate: how many  a/v frame per second
        # label rate: how many label per second
